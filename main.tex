...
Following the characteristic-strain construction used in \citet{delaurentiis2025}, we define LISA entry as the first upward crossing of the $n=2$ characteristic strain and the sky-averaged LISA characteristic-noise curve. The observed frequency is $f=2f_{\rm bin}/(1+z)$, and
\begin{equation}
...
\autoref{fig} shows representative $M_{b,0}=10^6,M_\odot$ binaries at $z=1$ with $q_{b,0}=e_{b,0}=0.8$. Systems beginning closer to the LISA band accrete very little and enter with essentially their initial mass ratio. Systems beginning farther out remain coupled to the gas for longer, gain more mass, and evolve farther toward equal mass. For $\dot m=0.3$, the representative tracks enter with $q_{b,\rm LISA}=0.801$--$0.952$ after growing by factors of $1.004$--$2.059$. For $\dot m=1$, they enter with $q_{b,\rm LISA}=0.803$--$0.971$ after growing by factors of $1.014$--$2.571$. Their eccentricities at the sensitivity-curve crossing span approximately $1.8\times10^{-3}$--$6.0\times10^{-3}$.

The open circles in \autoref{fig} mark a source-frame time of $30,{\rm Myr}$ when that time is reached before LISA entry. This is a diagnostic marker rather than an assumed disk lifetime. The stars show the limiting continuous-supply case in which gas remains available until the sensitivity-curve crossing. Because continuous supply maximizes the available mass growth, these endpoints provide an upper limit on how strongly accretion can drive the binaries toward equal mass.
