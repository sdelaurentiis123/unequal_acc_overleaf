...
In order to illuminate the relationship between the accretion of the binary and the precession of the CBD we must first characterize the time-variable behavior and attributes of the disk. Namely, we focus on the eccentricity and precession of the gas annuli in the bulk of the disk. 
...
\begin{equation}
    M_j(t) = M_j(t=0) +  \sum_{0}^{t} \Delta m(t)
\end{equation}
...
In addition to preferential accretion, we also report the average rate of change of the mass ratio, $\langle \dot{q}_b \rangle$. Namely, we calculate $\dot{q}_b(t)$ as described in \autoref{eqn:q_dot} in \autoref{sec:methods}, make a time-cut at $3000\tau$ to ensure that early instabilities do not effect our results, and report the mean on the truncated time-series in units of $\dot{M}_b/M_b$ in \autoref{fig:qdot_heatmap}.
